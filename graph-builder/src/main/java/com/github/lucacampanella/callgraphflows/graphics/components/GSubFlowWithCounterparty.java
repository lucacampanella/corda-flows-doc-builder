...
public class GSubFlowWithCounterparty extends GBaseGraphicContainerComponent {
...
//        }
        mainSubFlow.setStart(getStartX(), getStartY());
        if(!hasCounterpartySubFlow()) {
            GBaseGraphicComponent blockingStatement =
                    mainSubFlow.computeDimsStartingFrom(g2, null);
            while(blockingStatement != null) {
                addToCurrY(blockingStatement.getHeight(g2));
                blockingStatement = mainSubFlow.computeDimsStartingFrom(g2, blockingStatement);
            }
            setCurrY(mainSubFlow.getCurrY());
        }
        else {
            int counterpartyStartX = getStartX() + mainSubFlow.getDimensions(g2).width + SPACE_BETWEEN_FLOWS;
            ...
            counterpartySubFlow.setStart(counterpartyStartX, counterpartyStartY);
            GBaseGraphicComponent blockingLeft =
                    mainSubFlow.computeDimsStartingFrom(g2, null);
            GBaseGraphicComponent blockingRight =
                    counterpartySubFlow.computeDimsStartingFrom(g2, null);

            while(blockingLeft != null && blockingRight != null) {
                if(blockingLeft.isBrotherWith(blockingRight)) {
                    int currY = Math.max(blockingLeft.getStartY(), blockingRight.getStartY());
                    blockingLeft.setStartY(currY);
                    blockingRight.setStartY(currY);
                    blockingLeft = mainSubFlow.computeDimsStartingFrom(g2, blockingLeft);
                    blockingRight = counterpartySubFlow.computeDimsStartingFrom(g2, blockingRight);
                }
                else {
                    blockingLeft = mainSubFlow.computeDimsStartingFrom(g2, blockingLeft);
                }
